...
namespace UTNMdq2014
{
    public class FacultadDbContext : DbContext
    {
        ...
        protected override void OnModelCreating(DbModelBuilder modelBuilder)
        {
            ...
            //modelBuilder.Entity<Requisito>().HasRequired<Materia>(x => x.Referida).WithRequiredPrincipal();

            base.OnModelCreating(modelBuilder);
        }

        public FacultadDbContext()
            : base()
        {
    ...
    public class FacultadDbInitializer : DropCreateDatabaseAlways<FacultadDbContext>
    {
        /*protected override void Seed(FacultadDbContext context)
        {
            ...
            // Materia de prueba
            Materia materiaTest = new Materia("Calculo II", 2, 120);
            materiaTest.Requisitos.Add(requisitoTest);

            // Plan de estudio de prueba
            PlanEstudio planestudioTest = new PlanEstudio();
            planestudioTest.AgregarMateria(materiaTest);
            planestudioTest.AgregarMateria(materiaReqTest);
            ...
            context.PlanesEstudio.Add(planestudioTest);
            context.SaveChanges();

            context.Requisitos.Add(requisitoTest);
            context.SaveChanges();

            context.Materias.Add(materiaReqTest);
            context.SaveChanges();
